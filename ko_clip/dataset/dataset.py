...
class Text_Dataset(Dataset) :
    def __init__(self, data, diff, CONFIG):
        super().__init__()
        self.data = data
        self.ko_tokenizer = CLIPTokenizer.from_pretrained(CONFIG.TOKENIZER_MODEL_NAME)
        
        if CONFIG.TRAINING_TYPE == "default" :
            self.en_tokenizer = diff.tokenizer
        else :
            self.en_tokenizer = diff.tokenizer_2
...
def get_dataloader(CONFIG, diff) :
    ...
    data = data.sample(1602418, random_state=CONFIG.SEED)
